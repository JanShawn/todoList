...
*/
//*-----Function-----*//
function getTimeRemaining(endtime) {//取得剩餘時間
...
function getRenderContent(obj) { //取得渲染內容
  var result = {};
  var typeTxt = '';
  var remainTime = getTimeRemaining(obj.deadline);//取得剩餘時間
  switch (obj.type) {
    case "emergent_Important": typeTxt = '緊急重要'; break;
    case "emergent_N-Important": typeTxt = '緊急不重要'; break;
    case "N-emergent_Important": typeTxt = '不緊急重要'; break;
    case "N-emergent_N-Important": typeTxt = '不緊急不重要'; break;
  }
...
var todosModal = {
  ...
  addTodo: function (todoObj) {//新增清單代辦事項
    this.todoList.push(todoObj);
    //localStorage
    todosModal.setLocalStorage(this.todoList);
  },
  changeTodo: function (sn, todoObj) {//修改清單代辦事項
    var index = this.todoList.findIndex(todo => todo.sn == sn);
    this.todoList[index] = todoObj;
    //localStorage
    todosModal.setLocalStorage(this.todoList);
  },
  deleteTodo: function (sn) {//刪除清單代辦事項
    var index = this.todoList.findIndex(todo => todo.sn == sn);
    this.todoList.splice(index, 1);
    //localStorage
    todosModal.setLocalStorage(this.todoList);
  },
  toggleTodo: function (sn) {//更改清單狀態
    var index = this.todoList.findIndex(todo => todo.sn == sn);
...
//*-----view-----*//
var view = {
  displayTodos: function (list, sn) {//sn->編輯||新增的sn
    var tbody = document.querySelector('#tableTbody');
    tbody.innerHTML = "";//清空
    var todo = list;
    var totalTr = '<tr data-content="none"><td colspan="6" class="nothingTodo">無待辦事項</td></tr>';
    if (todo.length != 0) {
      totalTr = todo.sort((a, b) => Date.parse(a.deadline) - Date.parse(b.deadline)).map((obj, index) => {
        var renderContent = getRenderContent(obj);
        return `<tr data-index="${index}" data-sn="${obj.sn}" >` +
  ...
  setTrEventHandler: function () {//給tr中按鈕事件
    // completeBtn 完成按鈕
    var completes = document.querySelectorAll('.complete');//對象
    completes.forEach(btn => {//事件
    ...
    trashCans.forEach(btn => {//事件
      btn.addEventListener('click', function (e) {
        ...
        var todoObj = activeList[trIndex];
        Swal.fire({
          icon: 'warning', title: '你確定要刪除嗎?', text: "刪除之後不可復原喔!",
          ...
          if (result.isConfirmed) {
            // 刪除按鈕 //?需要問的地方
            controller.deleteTodo(todoObj.sn,
              Swal.fire(
                { title: '刪除!', text: '代辦事項已被刪除.', icon: 'success' },
              ).then((result) => {
  ...
  showAlert: function (icon, title, msg) {//跳錯誤
    return Swal.fire({ icon: icon, title: title, html: msg, confirmButtonText: 'OK' });
  },
  showTodoForm: function (type, obj) {
    ...
    $('#completeDate').data("DateTimePicker").date(overTime > 0 ? new Date(obj.deadline) : new Date());
...
//taskType(種類選擇)
var typeSpan = document.querySelectorAll(".taskType>span");//對象
typeSpan.forEach(span => span.addEventListener('click', function (e) {//事件
  //css
  typeSpan.forEach(type => {
  ...
  if (todoObj.sn == 0) {//sn=0表示新增
    ...
    for (let index = 0; index < sortList.length; index++) {
      ...
      if (obj.sn == todosModal.todoList.length) {
        todoObj.sn = obj.sn + 1; break;
      } else {
        if (obj.sn != index + 1) { todoObj.sn = index + 1; break; }
      }
    }
